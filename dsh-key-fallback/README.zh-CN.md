...
- **声明 DSH 版本支持**（2026-09-24）：新增 `@deepseek-ai/dsh` peer 条目（`0.1.7-rc.1`，逐版本枚举），
  把「支持的运行时」写成显式契约。该声明由 DSH 自己的 `evaluatePluginCompatibility()`（`dsh-app-boot` 的
  公开导出）在**安装时**与**每次启动时**校验：命中未实测的新运行时，bundle 会被**优雅跳过**
  （stderr 打 `skipping profile bundle`，DSH 照常启动），而不是以某种未定义方式崩掉。这个门禁
  **只有 DSH 0.1.7 起才存在**，所以 `≤0.1.6` 的运行时只是看到一条不认识的 peer，不受影响
  （本插件在那些版本上本来就走 `settings.yaml` 文件后端，一直可用）。只列实测版本，不用开放范围（仓库规范 3）。
