...
export function apply(ctx, entryConfig) {
  // 启动标记：v3 加载时立即打 INFO，DSH 启动日志能直接确认 v3 跑没跑
  try { ctx.logger.info('key-fallback: v3 host apply() starting (NS=key-fallback-pools)') } catch (e) {}
  ...
  try {
    ...
    try { ctx.logger.info('key-fallback: v3 settings registered, providers count=' + Object.keys(scope.get().providers || {}).length) } catch (e) {}
  } catch (e) {
  ...
  async function safeUpdate(patch) { try { if (!scope || typeof scope.update !== 'function') { ctx.logger.warn('key-fallback: scope not initialized, skip update'); return } return await scope.update(patch) } catch (e) { ctx.logger.warn('key-fallback: scope.update failed: %s', (e && e.message) || e) } }
    // settings 更新（useKeyId / enabled / keys 等改变）时清内存 state，让下次请求立刻用新配置
  scope.watch(() => {
    try {
      states.clear()
    } catch (e) {}
