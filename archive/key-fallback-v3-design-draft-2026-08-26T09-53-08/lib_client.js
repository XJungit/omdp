  ...
  factory: function (require) {
    ...
    function KeyRow(props) {
      ...
      function save() {
        ...
        setBusy(true); setErr('')
        try {
          __kfReportDiag('save', 'PATCH start provider=' + props.provider + ' keyId=' + k.id)
        } catch (e) {}
        api('/keys', { method: 'PATCH', body: { provider: props.provider, keyId: k.id, value: value } })
          .then(function (r) {
            setBusy(false)
            try { __kfReportDiag('save', 'PATCH done ok=' + r.ok + ' status=' + r.status) } catch (e) {}
            if (!r.ok) { setErr((r.body && r.body.error) || ('HTTP ' + r.status)); return }
            setValue(''); setEditing(false); props.onChanged()
          })
          .catch(function (e) { setBusy(false); try { __kfReportDiag('save', 'PATCH catch ' + String(e)) } catch (x) {}; setErr(String(e && e.message || e)) })
      }
      var isEnv = props.source === 'env'
      return ce('div', { className: 'kf_keyRow' },
        ce('div', { className: 'kf_keyLabel' },
          k.label ? k.label : (k.id || '').slice(0, 8),
          isEnv ? ce('span', { className: 'kf_badge brand' }, '[环境来源]') : null,
          ce('span', { className: 'kf_keyRef' }, k.ref)
        ),
        isEnv
          ? ce('span', { className: 'kf_hiddenBadge', style: { flex: 1 } }, '来自环境配置（不可编辑/删除，参与轮换）')
          : isEditing
            ? ce('div', null,
    ...
    function PoolCard(props) {
      ...
      function deleteKey(keyId) {
        ...
        api('/keys?provider=' + encodeURIComponent(pool.provider) + '&keyId=' + encodeURIComponent(keyId), { method: 'DELETE' })
          .then(function (r) { if (r.ok) props.onChanged(); else setErr((r.body && r.body.error) || ('HTTP ' + r.status)) })
          .catch(function (e) { setErr(String(e && e.message || e)) })
      }
      function setUseKey(keyId) {
        api('/pools', { method: 'POST', body: { provider: pool.provider, enabled: pool.enabled, env: pool.env, cooldownMs: pool.cooldownMs, rotateOn: pool.rotateOn, useKeyId: keyId } })
      ...
      return ce('div', { className: 'kf_card' },
        ...
        ce('div', { style: { fontSize: 12, color: 'var(--dsw-alias-label-secondary,#9aa)', display: 'flex', gap: 14, flexWrap: 'wrap', alignItems: 'center' } },
          ...
          ce('label', { style: { display: 'flex', alignItems: 'center', gap: 6 } },
            ce('span', null, '当前使用:'),
            ce('select', { value: pool.useKeyId || '_auto', onChange: function (e) { var v = e.target.value; if (v === '_auto') setUseKey(''); else setUseKey(v) } },
              ce('option', { value: '_auto' }, '自动轮换'),
              (Array.isArray(pool.keys) ? pool.keys : []).map(function (k) {
                return ce('option', { value: k.id, key: k.id }, (k.source === 'env' ? '[环境] ' : '') + (k.label || k.id.slice(0, 8)))
              }),
            ),
          ),
        ),
        ...
        ((Array.isArray(pool.keys) ? pool.keys : [])).length === 0
          ...
          : ce('div', { className: 'kf_keys' }, (Array.isArray(pool.keys) ? pool.keys : []).map(function (k) {
                return ce(KeyRow, { key: k.id, entry: k, provider: pool.provider, onDelete: (k.source === 'env') ? null : function () { deleteKey(k.id) }, onChanged: props.onChanged, source: k.source || 'user' })
              }),
            ),
