// @omdp/dsh-gitbash-win — host half.
// ============================================================
// Registers a global `gitbash` tool for DeepSeek Harness on Windows.
...
/** Resolve the Git Bash executable: GIT_BASH env, known paths, then PATH. */
function resolveGitBashStrict() {
  const fromEnv = process.env.GIT_BASH
  if (fromEnv && candidateExists(fromEnv)) return fromEnv
...
function gitBashDescription(escalationModes) {
  const base =
    'Execute a Git Bash command (`bash -c`) and return its stdout/stderr.\n\nDEFAULT SHELL ON WINDOWS — Git Bash is the preferred shell for routine work. Use it for: file operations (ls / cp / mv / rm / find), text processing (grep / sed / awk / cut / tr), archives (tar / zip / unzip), process checks (ps / kill), git, and downloads/curl. It gives a true POSIX environment (pipes, globs, standard Unix semantics) that also transfers cleanly to Linux and macOS.\n\nUse `pwsh` ONLY for Windows-specific operations with no Bash equivalent: COM objects, Windows services, the registry, or .NET cmdlets. Do not reach for pwsh by default for ordinary file/text work.\n\nEach call runs in a fresh shell: no state (cwd, variables, functions) persists between calls — pass `workdir` instead of using `cd`. Paths accept native Windows form (`C:\\...`) and Git Bash form (`/c/...`).\n\nNon-zero exits are reported as `[exit code: N]`. Current harness environment facts are exposed through managed `$DSH_*` variables; inspect them when needed. Commands may run under a file sandbox; a blocked file operation is reported as `[sandbox: file access denied under <mode> mode]` — a policy denial, not a bug; do not retry another way. Long output is truncated to its tail; the full output is saved to a file whose path is reported when available. On Windows a force-killed command settles as `[exit code: 1]` without a signal marker — treat it as an interruption, not a command failure. Set `run_in_background: true` for long-running commands: the call returns a job id immediately; read its output with `job_output` and stop it with `job_kill`.'
...
export async function apply(ctx, config = {}) {
  ...
  if (bash === undefined) {
    throw new Error('tool-gitbash: Git Bash not found — checked ' + GIT_BASH_CANDIDATES.join(', ') + '. Install Git for Windows (https://git-scm.com/download/win) or set GIT_BASH to your bash.exe.')
  }
